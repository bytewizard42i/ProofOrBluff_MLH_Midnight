...
The newbie-friendly health-check reference (cheat-sheet table, the
`POST is required` mystery solved, copy-pasteable curl queries,
which-port-looks-broken-but-isn't) lives in
[`FOR_JUDGES.md`](../../FOR_JUDGES.md) at the repo root. It's a single
page, written for someone running the demo for the first time.

The rest of this guide is the **developer's** reference — seeds,
funding flows, SDK matrix, troubleshooting deep cuts.
